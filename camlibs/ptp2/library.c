...
#define NIKON_1_ADDITIONAL_DEVPROPS 29
	...
	if (di->VendorExtensionID == PTP_VENDOR_SONY) {
		...
		if (ptp_operation_issupported(&camera->pl->params, PTP_OC_SONY_GetSDIOGetExtDeviceInfo)) {
			...
			propval.u8 = 1; /* application */
			LOG_ON_PTP_E (ptp_sony_setdevicecontrolvaluea (params, PTP_DPC_SONY_PriorityMode, &propval, PTP_DTC_UINT8));
...
static struct {
  ...
  {"Sony:ILCE-7M5 (Control)",		0x054c, 0x0f56, PTP_CAP|PTP_CAP_PREVIEW},
	...
	/* Nikon Z5 II, USB PID 0x0456, iProduct "NIKON DSC Z5_2" */
	{"Nikon:Z5_2",                    0x04b0, 0x0456, PTP_CAP|PTP_CAP_PREVIEW},
	...
	*/
	...
	{"Canon:EOS R6 Mark II",			0x04a9, 0x330b, PTP_CAP|PTP_CAP_PREVIEW},
...
// Ben G - Sept 15, 2021
// Bare bones canon trigger replacing original below
// this implementation doesn't check for events or
...
camera_trigger_canon_eos_capture (Camera *camera, GPContext *context)
{
	...
	// printf("camera_trigger_canon_eos_capture\n");

	if (!params->eos_captureenabled) {
		// printf("camera_prepare_capture\n");
		camera_prepare_capture (camera, context);
	} else {
...
*/
				...
				case PTP_CANON_EOS_CHANGES_TYPE_FOCUSINFO:
					*eventtype = GP_EVENT_UNKNOWN;
					C_MEM (*eventdata = malloc(strlen("Focus Info 12345678901234567890123456789abc")+1));
